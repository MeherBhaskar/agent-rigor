...
1. **Stall & Doom Loop Resolution**
   ...
   - Execute `17_recursive_self_correction.md` if an approach fails 3 times or divergence is detected.

2. **Scope Defense**
   - Triggered when starting any task or when discovering adjacent issues.
   - Execute `18_scope_containment.md` to enforce the touch audit and log deferred observations.

3. **Knowledge Consolidation**
   - Triggered after major failures, milestones, or 3-strike events.
   - Execute `19_experiential_consolidation.md` to extract reusable procedural rules and store them in `.docs/learned_rules/`.

4. **Meta-Orchestration**
   - Triggered at the absolute beginning of a complex, multi-task goal.
   - Execute `20_cascade_orchestration.md` to act as the overarching conductor, tracking progress across all other phases.
